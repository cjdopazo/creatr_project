...
function undoChange() {
  ...
  console.log("this.quill.history:", this.quill?.history);
  
  if (this.quill && this.quill.history) {
    console.log("Calling undo...");
    this.quill.history.undo();
  } else {
    console.error("Quill or history not available");
  }
}
...
// Función para registrar los iconos personalizados de undo/redo
const registerQuillIcons = () => {
  if (typeof window !== "undefined") {
...
export default function PostEditorContent({
  ...
  // Registrar los iconos cuando el componente se monta
  useEffect(() => {
    console.log("Registering Quill icons...");
    registerQuillIcons();
  }, []);
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="space-y-5">
          ...
          <div>
            {!hasContent ? (
              <Button
                ...
                variant="outline"
                size="sm"
                className="border-purple-600 text-lime-300 hover:bg-purple-500 hover:text-white disabled:opacity-50 w-full"
              >
          ...
          {(isGenerating || isImproving) && (
            <BarLoader width={"95%"} color="#52d404" />
          )}
